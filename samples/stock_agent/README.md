...
> 说明：这只是规则型投资辅助工具，不构成投资建议。实时模式下，美股和港股行情来自 Yahoo Finance，A 股行情和分红数据来自东方财富；A 股和港股代码会自动转换成对应数据源使用的代码格式。
...
- 切换离线样例行情或实时行情（A 股走东方财富，美股/港股走 Yahoo Finance）。
...
- 美股、港股：Yahoo Finance quote API。
- A 股：东方财富 quote API 获取最新价和 PE，东方财富 F10 分红融资数据解析最近现金分红并计算股息率。
